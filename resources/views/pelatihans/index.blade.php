...
<div class="space-y-6 pb-12">
    ...
    @if(session('success'))
        <div class="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200/80 text-emerald-800 text-xs font-semibold flex items-center gap-3 shadow-xs">
            <span class="w-6 h-6 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0 text-xs">
        ...
        <div class="overflow-x-auto custom-scrollbar">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-xs font-medium text-slate-700">
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors group">
                            ...
                            <td class="py-4 px-4 align-middle">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 font-bold text-xs group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-xs">
                                        <i class="fa-solid fa-graduation-cap"></i>
                                    </div>
                                    <a href="{{ route('pelatihans.show', $pelatihan) }}" class="font-bold text-slate-800 hover:text-indigo-600 transition-colors line-clamp-2 leading-snug">
                            ...
                            {{-- Tempat --}}
                            <td class="py-4 px-4 whitespace-nowrap align-middle">
                                <span class="px-2.5 py-1 bg-slate-100 text-slate-600 font-semibold rounded-md text-[11px] inline-flex items-center gap-1.5">
                            ...
                            <td class="py-4 px-4 whitespace-nowrap align-middle">
                                <div class="space-y-1">
                                    <div class="flex justify-between items-center text-[11px]">
                                        <span class="font-black text-indigo-600">{{ $pelatihan->persen }}%</span>
                                        <span class="text-[10px] text-slate-400 font-semibold">
                                            {{ $pelatihan->persen == 100 ? 'Selesai' : 'Progress' }}
                                        </span>
                                    </div>
                                    <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                                        <div class="bg-gradient-to-r from-indigo-500 to-sky-400 h-full rounded-full transition-all duration-500" 
                                             style="width: {{ $pelatihan->persen }}%"></div>
                                    </div>
                                </div>
                            </td>
                            ...
                            @auth
                                @if(auth()->user()->role == 'admin')
                                    ...
                                    <td class="py-4 px-4 text-center whitespace-nowrap align-middle">
                                        <form action="{{ route('pelatihans.favorite', $pelatihan) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center transition-all group/star" title="Tandai Favorit">
                                                @if($pelatihan->favorit)
                                                    <i class="fa-solid fa-star text-amber-400 text-sm group-hover/star:scale-110 transition-transform"></i>
                                                @else
                                                    <i class="fa-regular fa-star text-slate-300 group-hover/star:text-amber-400 text-sm group-hover/star:scale-110 transition-all"></i>
                                                @endif
                                            </button>
                                        </form>
                                    </td>
                                    ...
                                    <td class="py-4 px-4 text-center whitespace-nowrap align-middle">
                                        <div class="flex items-center justify-center gap-1.5">
                                            ...
                                            <form action="{{ route('pelatihans.destroy', $pelatihan) }}" method="POST" 
                                                  onsubmit="return confirm('Yakin ingin menghapus pelatihan ini?')" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ auth()->check() && auth()->user()->role == 'admin' ? 9 : 7 }}" class="py-12 text-center bg-white">
                                <div class="max-w-xs mx-auto text-center space-y-2">
                                    ...
                                    <p class="text-[11px] text-slate-400">Tidak ada data yang cocok dengan kriteria pencarian Anda.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
