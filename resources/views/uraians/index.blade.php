...
@section('content')
    ...
    <table class="w-full text-left border-collapse align-middle">
        <thead>
            <tr class="bg-slate-50/80 border-b border-slate-200 text-[11px] uppercase font-bold tracking-wider text-slate-500">
            ...
            @forelse($uraians as $index => $uraian)
                <tr class="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td class="px-3 py-4 whitespace-nowrap">
                        ...
                        @endphp
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ring-inset {{ $badgeTahapan }}">
                            {{ $uraian->tahapan }}
                        </span>
                    </td>
                    ...
                    <td class="px-3 py-4 whitespace-nowrap text-slate-600 text-xs">
                        ...
                        @if($uraian->tanggal_selesai)
                            <div class="text-[11px] text-emerald-600 mt-1 font-medium flex items-center gap-1">
                                ...
                                <span>Selesai: {{ \Carbon\Carbon::parse($uraian->tanggal_selesai)->translatedFormat('d M Y') }}</span>
                            </div>
                        @endif
                    </td>
                    ...
                    <td class="px-3 py-4 text-center whitespace-nowrap">
                        ...
                        @endphp
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ring-inset {{ $badgeStatus }}">
                    ...
                    {{-- PIC --}}
                    <td class="px-3 py-4 whitespace-nowrap">
                        <span class="inline-block bg-slate-100 text-slate-700 px-2.5 py-1 rounded-lg text-xs font-medium">
                            {{ $uraian->pic }}
                        </span>
                    </td>
                    ...
                    <td class="px-3 py-4 whitespace-nowrap">
                        @if($uraian->lampiran)
                            <a href="{{ route('uraians.show', $uraian) }}"
                                ...
                                <i class="fa-solid {{ $uraian->fileIcon() }} text-sky-500 group-hover:scale-110 transition-transform"></i>
                            ...
                            @auth
                                @if(in_array(auth()->user()->role, ['admin', 'superadmin']))
                                    <div class="upload-wrapper">
                                        <input type="file" class="hidden upload-input" data-url="{{ route('uraians.upload', $uraian) }}">
                                        <button type="button"
            ...
            @empty
                <tr>
                    <td colspan="{{ auth()->check() && in_array(auth()->user()->role, ['admin', 'superadmin']) ? 9 : 8 }}"
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Uraian kegiatan untuk pelatihan ini belum ditambahkan.</p>
                    </td>
                </tr>
            @endforelse
        </tbody>
